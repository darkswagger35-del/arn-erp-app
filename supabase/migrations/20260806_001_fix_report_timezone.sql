...
-- Türkiye yerel tarih sınırlarını UTC'ye dönüşürken önceki güne kaydırmamak için
-- rapor detay fonksiyonundaki geçmiş kayıt tarih filtresini düzeltir.
...
as $$
  ...
  historical_rows as (
    select
      ...
      h.id record_id,
      (h.transaction_date::timestamp at time zone 'Europe/Istanbul') transaction_date,
      h.customer_id,
      coalesce(nullif(btrim(c.full_name),''), nullif(btrim(c.company_name),''), 'Müşteri') customer_name,
    ...
    where h.company_id=public.current_company_id()
      and h.transaction_date >= (p_start at time zone 'Europe/Istanbul')::date
      and h.transaction_date <  (p_end   at time zone 'Europe/Istanbul')::date
  )
